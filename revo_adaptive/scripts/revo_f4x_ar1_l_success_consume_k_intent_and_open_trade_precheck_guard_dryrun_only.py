...
TARGET_REL = "scripts/revo_f4x_l_paper_bridge_execution_sandbox_dryrun.py"
ORIG_REL = "scripts/revo_f4x_l_paper_bridge_execution_sandbox_dryrun_PRE_AR1_ORIGINAL.py"
MARKER = "CONTROL_TOWER_F4X_AR1_L_WRAPPER"
...
def main():
    ...
    original = repo / ORIG_REL
    ts = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    backup = target.with_suffix(target.suffix + f".F4X_AR1_BACKUP_{ts}")

    actions = []
    failures = []

    if not target.exists():
        failures.append("TARGET_L_SCRIPT_MISSING")
    else:
        text = target.read_text(encoding="utf-8", errors="replace")
        if MARKER in text:
            actions.append("ALREADY_PATCHED")
        elif args.execute:
            shutil.copy2(target, backup)
            actions.append(f"BACKUP_CREATED:{backup}")
            if not original.exists():
                shutil.copy2(target, original)
                actions.append(f"ORIGINAL_CREATED:{original}")
            target.write_text(WRAPPER, encoding="utf-8")
            target.chmod(0o755)
            actions.append("WRAPPER_INSTALLED")

            compile_target = subprocess.run(["python3", "-m", "py_compile", str(target)], capture_output=True, text=True)
            compile_original = subprocess.run(["python3", "-m", "py_compile", str(original)], capture_output=True, text=True)

            if compile_target.returncode != 0:
                failures.append("WRAPPER_COMPILE_FAILED")
                failures.append(compile_target.stderr[-1000:])
            if compile_original.returncode != 0:
                failures.append("ORIGINAL_COMPILE_FAILED")
                failures.append(compile_original.stderr[-1000:])
        else:
            actions.append("DRYRUN_ONLY_NO_WRITE")

    final_decision = "F4X_AR1_PATCHED_L_WRAPPER_INSTALLED_RESTART_NOT_REQUIRED"
    if failures:
        final_decision = "F4X_AR1_ABORTED_GUARD_FAILED"
    elif not args.execute:
        final_decision = "F4X_AR1_DRYRUN_READY_EXECUTE_REQUIRED"
    elif "ALREADY_PATCHED" in actions:
        final_decision = "F4X_AR1_ALREADY_PATCHED"
    ...
    result = {
        ...
        "decision_policy": [
            "Patch L only.",
            "Before forceenter, block same-pair open trade.",
            "After successful dry-run order, consume K active signal.",
            "No live. No risk-up. No gate-loosen.",
        ],
    }
    ...
    lines = [
        "F4X_AR1_L_SUCCESS_CONSUME_K_INTENT_AND_OPEN_TRADE_PRECHECK_GUARD_DRYRUN_ONLY_COMPACT",
        ...
        *(failures or ["NONE"]),
        ...
        f"backup={backup}",
        "DECISION_POLICY",
        "Patch L only. No strategy gate unlock. No live. No risk-up. No gate-loosen.",
        "OUTPUT_FILES",
        f"full_json={full}",
        f"compact={compact}",
        f"active={active}",
