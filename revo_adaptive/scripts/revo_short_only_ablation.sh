#!/bin/bash
# Short-only MR ablation: fixed vs atr2 vs atr3 (best from prior)
# Uses RevoAdaptiveBacktestShortOnly (short MR with same score gates)
set -u
CFG="/freqtrade/user_data/configs/config.bybit.backtest.static.json"
STRAT="RevoAdaptiveBacktestShortOnly"
OUT="/freqtrade/user_data/logs/revo_short_only_ablation.jsonl"
...
run_one() {
  ...
  freqtrade backtesting --config "$CFG" --strategy "$STRAT" \
    --timerange "$tr" --cache none --export trades >/tmp/ft_short_${name}.log 2>&1
  local rc=$?
  local zip
  ...
  if [ $rc -ne 0 ] || [ -z "$zip" ]; then
    ...
    tail -n 20 /tmp/ft_short_${name}.log >&2 || true
  else
...
echo "SHORT ONLY ABLATION DONE -> $OUT" >&2
cat "$OUT"