#!/bin/bash
# BTC regime gate + flow-gate ablation: pure / scoring / block_danger / hard
# Uses RevoAdaptiveBacktestBTCGateAblation (proxy flow + BTC 15m VWAP regime, no lookahead)
# One env change per run: REVO_FLOW_GATE_MODE
set -u
CFG="/freqtrade/user_data/configs/config.bybit.backtest.static.json"
STRAT="RevoAdaptiveBacktestBTCGateAblation"
OUT="/freqtrade/user_data/logs/revo_btc_gate_ablation.jsonl"
RESDIR="/freqtrade/user_data/backtest_results"
EXTRACT="/freqtrade/user_data/scripts/revo_extract.py"
mkdir -p "$(dirname "$OUT")"
: > "$OUT"

# Live-matched entry quality
...
run_one() {
  ...
  freqtrade backtesting --config "$CFG" --strategy "$STRAT" \
    --timerange "$tr" --cache none --export trades >/tmp/ft_btc_ablation_${name}.log 2>&1
  local rc=$?
  local zip
  ...
  if [ $rc -ne 0 ] || [ -z "$zip" ]; then
    ...
    tail -n 20 /tmp/ft_btc_ablation_${name}.log >&2 || true
  else
...
# IS ~ Jun1-Jun25 | OOS Jun25-Jul10 | FULL Jun1-Jul10
...
echo "BTC GATE ABLATION DONE -> $OUT" >&2
cat "$OUT"